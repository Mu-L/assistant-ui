...
const ThreadListItemGroups: FC<{ searchQuery?: string }> = ({
  ...
  const dates = useMemo(
    () => threadIds.map((id) => itemsById.get(id)?.lastMessageAt),
    [threadIds, itemsById],
  );

  const filteredIndices = useMemo(() => {
    if (!query) return threadIds.map((_, index) => index);
    return threadIds
      .map((id, index) => ({ id, index }))
      .filter(({ id }) =>
        (itemsById.get(id)?.title ?? "").toLowerCase().includes(query),
      )
      .map(({ index }) => index);
  }, [threadIds, itemsById, query]);

  const groups = useMemo<ThreadListGroup[] | null>(() => {
    if (!filteredIndices.some((index) => dates[index])) return null;
